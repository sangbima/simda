...
use app\models\BatchKibbSearch;
...
/**
 * BatchKibbController implements the CRUD actions for BatchKibb model.
 */
class BatchKibbController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    /**
     * Lists all BatchKibb models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new BatchKibbSearch();
    ...
    /**
     * Displays a single BatchKibb model.
     * @param integer $id
     * @return mixed
     */
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->expired_date = $model->expired_date ? date('Y-m-d', strtotime($model->expired_date)) : null;
            ...
            if($model->validate()) {
                ...
                $model->expired_date = $model->expired_date ? date('d-m-Y', strtotime($model->expired_date)) : null;
    ...
    /**
     * Updates an existing BatchKibb model.
    ...
    /**
     * Deletes an existing BatchKibb model.
    ...
    protected function autoNumber($code_main)
    {
        ...
        $model = BatchKibb::find()->where(['code_main' => $code_main])->orderBy(['code_start_num' => SORT_DESC])->one();
        if ($model) {
    ...
    public function getRoom($gov_unit_id)
    {
        $datas =  \app\models\Room::find()->where(['gov_unit_id' => $gov_unit_id])->all();
    ...
    /**
     * Finds the BatchKibb model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return BatchKibb the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = BatchKibb::findOne($id)) !== null) {
            return $model;
        } else {
